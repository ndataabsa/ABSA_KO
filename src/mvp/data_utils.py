...
def read_line_examples_from_file_para(data_path, lowercase, silence=True):
    """
    ...
    if silence:
        print(f"Method: Paraphrase, Total examples = {len(sents)}")
    return sents, labels
...
def get_para_aste_targets_para(sents, labels):
    targets = []
...
def get_para_tasd_targets_para(sents, labels):
...
def get_para_asqp_targets_para(sents, labels):
    """
    ...
    for label in labels:
        ...
        for quad in label:
            ...
            one_quad_sentence = f"{ac} 는(가) {man_ot} 때문에 {at} 는(가) {ot}"
...
def get_transformed_io_para(data_path, data_name, data_type, args):
    """
    The main function to transform input & target according to the task
    """
    sents, labels = read_line_examples_from_file_para(data_path, args.lowercase)

    # the input is just the raw sentence
    inputs = [s.copy() for s in sents]

    if args.task == 'aste':
        targets = get_para_aste_targets_para(sents, labels)
    elif args.task == 'tasd':
        targets = get_para_tasd_targets_para(sents, labels)
    elif args.task == 'asqp':
        targets = get_para_asqp_targets_para(sents, labels)
    ...
    def _build_examples(self):
        ...
        if self.args.multi_task:
            ...
            if self.args.paraphrase:
                inputs, targets = get_transformed_io_para(self.data_path,
                                                     self.task_name,
                                                     self.data_name,
                                                     self.args)
            else:
